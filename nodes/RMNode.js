    ...
    function broadlinkNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            _device.on("deviceReady", (devm) => {
                ...
                var _config = { action: config.action, remote: config.remote, button: config.button, repeat: config.repeat, RFSweep: config.RFSweep, data: undefined };
                if (_config.action == "_msg_") { _config.action = msg.payload.action; _config.remote = msg.payload.remote; _config.button = msg.payload.button; _config.repeat = msg.payload.repeat; _config.RFSweep = msg.payload.RFSweep; _config.data = (msg.payload.data != undefined && typeof (msg.payload.data) == "string") ? JSON.parse(msg.payload.data) : ((msg.payload.data != undefined && typeof (msg.payload.data) == "object") ? msg.payload.data : undefined); }
                    ...
                    case "send":
                        if (_config.data === undefined) {
                            ...
                            fs.readFile(conf.folder + "/jsonIrCode", options, function (err, data) {
                                ...
                                var code = new Buffer(JSON.parse(data).filter(function (obj) { if (obj.buttonId == _config.button) { return true; } })[0].code);
